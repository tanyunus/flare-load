...
// ── Config ────────────────────────────────────────────────────────────────────
const FP_SEED_CF          = 2500;
const FP_SEED_LOCAL       = 2500;
const FP_SEED_META        = '_fp_seed';
const FP_SEED_CF_DELAY_US = 250000; // 250 ms between CF uploads ≈ 4 req/s

// ── Dispatch ──────────────────────────────────────────────────────────────────
$mode = in_array('cleanup', array_slice($GLOBALS['argv'] ?? [], 1)) ? 'cleanup' : 'seed';
$mode === 'cleanup' ? fp_seed_cleanup() : fp_seed_run();

// ── Seed ──────────────────────────────────────────────────────────────────────
function fp_seed_run(): void
{
    $accountHash = get_option('fp_cf_account_hash', '');
    $accountId   = get_option('fp_cf_account_id', '');
    $apiToken    = get_option('fp_cf_api_token', '');
    ...
    WP_CLI::log('Phase 1/2 — Uploading ' . FP_SEED_CF . ' images to Cloudflare (~' . round(FP_SEED_CF * FP_SEED_CF_DELAY_US / 1e6 / 60, 0) . ' min)…');
    $bar = WP_CLI\Utils\make_progress_bar('CF uploads', FP_SEED_CF);

    for ($i = 1; $i <= FP_SEED_CF; $i++) {
        $filename = "fp-seed-cf-{$i}.jpg";
        ...
        try {
            ...
            $bar->tick();
            usleep(FP_SEED_CF_DELAY_US);
            continue;
        }

        $cfId      = $result['result']['id'];
        $cfBaseUrl = "https://imagedelivery.net/{$accountHash}/{$cfId}";
        $thumbPath = fp_seed_make_thumb($samplePath, $seedDir, "fp-seed-cf-thumb-{$i}.jpg");

        $postId = fp_seed_insert([
        ...
        wp_update_attachment_metadata($attachmentId, [
            ...
            'fp_cf_thumbnail' => $thumbPath ? ['path' => $thumbPath, 'width' => 100, 'height' => 100] : false,
    ...
    $bar->finish();
    WP_CLI::success('Phase 1 complete — ' . FP_SEED_CF . ' CF posts created.');

    // ── Phase 2: Local uploads ────────────────────────────────────────────────
    WP_CLI::log('Phase 2/2 — Creating ' . FP_SEED_LOCAL . ' local-upload posts…');
    $bar = WP_CLI\Utils\make_progress_bar('Local uploads', FP_SEED_LOCAL);

    $localDir = $uploadDir['basedir'] . '/fp-seed-local';
    $localUrl = $uploadDir['baseurl'] . '/fp-seed-local';

    if (!wp_mkdir_p($localDir)) {
        WP_CLI::error("Could not create local seed directory: {$localDir}");
    }

    for ($i = 1; $i <= FP_SEED_LOCAL; $i++) {
        $filename = "fp-seed-local-{$i}.jpg";
    ...
    $bar->finish();
    WP_CLI::success(
        'Seeding complete — ' . FP_SEED_CF . ' CF posts + ' . FP_SEED_LOCAL . ' local posts = ' .
        (FP_SEED_CF + FP_SEED_LOCAL) . ' total.'
    );
}
...
function fp_seed_cleanup(): void
{
    ...
    foreach ($ids as $id) {
        $cfId = get_post_meta((int) $id, 'fp_cf_image_id', true);
        if ($cfId) {
            fp_seed_cf_delete($cfId);
    ...
    $uploadDir = wp_upload_dir();
    foreach (['fp-seed', 'fp-seed-local'] as $dir) {
...
function fp_seed_insert(array $postarr)
{
...
function fp_seed_cf_delete(string $cfId): void
{
    try {
...
function fp_seed_make_thumb(string $src, string $dir, string $name): string|false
{
    $dest = $dir . '/' . $name;
